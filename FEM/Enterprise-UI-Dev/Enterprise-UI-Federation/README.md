...
- **Error boundary limitations:** Stop the remote dev server and reload the host — you'll see a blank white page, not the error boundary fallback. Check the console: you'll find `ERR_CONNECTION_REFUSED` on `mf-manifest.json` followed by a `Federation Runtime` error. This is expected behavior. Module Federation's manifest fetch is part of the eager shared module negotiation that runs *before* React mounts. When the remote is completely unreachable, the federation runtime throws before `createRoot` is ever called — so the React `ErrorBoundary` never gets a chance to render. To stop only the remote from a single terminal, run `lsof -ti:3001 | xargs kill` in a new terminal tab, then reload `http://localhost:3000`. The `ErrorBoundary` in `host/src/app.tsx` is still useful for cases where the remote module loads but throws during React rendering — it just can't recover from a total remote outage at the module infrastructure level.
